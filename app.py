...
def prepare_data(source_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Normalize and validate source rows once, then calculate metrics."""
...
st.sidebar.info(
    "Analysis level is fixed at one row per coil to keep all "
    "tables and charts consistent."
)
...
# All reports are calculated from one aggregated row per coil.
